...
The page checks modules, local JSON loading, storage, Canvas and Workers. Reopen it to verify persistent visit counters. Copy the App to another directory to check localStorage and IndexedDB isolation. The cookie counter is shared across Apps on the same host. Drag native files or text into the drop area to check system integration. Keep the window visible during animation checks.
