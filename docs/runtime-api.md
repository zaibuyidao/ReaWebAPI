...
`transaction` groups existing batch/Undo calls. The name does not promise atomic rollback or isolation: completed writes remain applied and other edits may interleave. See the [migration table](runtime-api-inventory.md) for the eight moved methods.
...
`await reaper.events.on(name, callback)` returns an async disposer. All existing event names remain supported; the flat JavaScript subscription method has been removed. Additional event names:

| Name | Meaning |
| --- | --- |
| `track-added`, `track-deleted` | Changes to current-project track GUID sets, after establishing a baseline |
| `track-selected` | The existing track-selection snapshot |
...
`events.off(name, callback)` returns `Promise<void>` and removes all registrations of that exact callback for that name, including pending registration; other callbacks/names are unchanged. Missing registrations are a no-op. Callback execution already in progress is not aborted. The individual disposer returned by `on()`/`onDrop()` remains supported. `debug.info` is removed; use `debug.log` (its stored level remains `info`).
