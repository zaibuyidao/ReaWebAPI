...
Windows、macOS 和 Linux 的浮动 Docker 标题栏跟随当前 WebView 的图标和显示设置。切换到其他标签、取消停靠或关闭后，还原容器原有图标状态。嵌入 REAPER 主窗口的 Docker 保留 WebView 的图标配置，不更改 REAPER 主窗口图标。Windows 设置窗口大小图标。macOS 使用窗口的文档代理图标，不更改 REAPER 的应用或 Dock 图标。Linux 设置窗口图标，通过窗口装饰提示请求移除标题栏图标，图标及占位是否移除取决于桌面主题和窗口管理器。停靠标签文字的显示由 REAPER 控制。这些 API 不修改 HTML 中的 favicon 声明。
...
`reaper.debug.openDevTools()` 在 Windows、macOS 和 Linux 上请求显示检查器。macOS 原生检查器控制不可用时以 `DEVTOOLS_UNAVAILABLE` 错误拒绝 Promise。快捷键、布局及 `getDiagnostics()` 的 `devtools` 字段见 [DevTools](devtools.zh-CN.md)。
