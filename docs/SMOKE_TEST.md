...
- Verify HTML `<title>`, dynamic `document.title`, title removal/blank fallback and Unicode titles in floating and docked windows. `setTitle()` must override later page changes and survive reload/instance reuse. Untitled pages retain the directory-based fallback without exposing `instanceKey`. The opt-in Windows check is `python tests/plugin_smoke.py --dll <extension> --webview --titles`.
