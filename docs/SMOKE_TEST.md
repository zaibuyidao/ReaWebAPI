...
For Windows Docker icons, build `windows_dock_icons` and install `reaper_dock_icon_test.dll` instead of the title test in a disposable profile. Create `dock-icon-test.enabled` and use the same launch settings. The test checks native small/large icon pixels, visibility, replacement while hidden, inactive tabs, two WebViews, close/reopen, favicon removal, container restoration, focus and bounds. Results go to `dock-icon-test.log`. Repeat with `dockermode0=0` to check embedded Docker state without changing REAPER's main icon. For visual inspection, create `dock-icon-test.pause` before starting and remove it to resume. Verify the floating Docker draws the icon both initially and after switching back from a native tab, restores the native tab's caption style, and retains working pin/close controls.
