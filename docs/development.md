...
Use `await reaper.debug.getDiagnostics()` to inspect backend, stage, queue counts and the last host error. On Windows/Linux, press Ctrl+Shift+I in the WebView to show or hide DevTools, or call `reaper.debug.openDevTools()` to request opening it. On macOS, the shortcut toggles a Safari setup guide and the API shows it. See [DevTools](devtools.md) for layouts, saved preferences and platform limits. Reopen the page after changing assets. Reloading creates a new document and invalidates its old handles.
...
`ReaWebAPI-ReaPack-v<version>.zip` contains `ReaWebAPI.ext` alongside `extension/` with seven native files and the complete `web/` Demo folder. The SDK ZIP contains development materials.

The descriptor lists every Demo file and installs them under `web/` in the package's Scripts directory. `ReaWebAPI_Demo.lua` is registered in the Action List. Other resources retain their relative paths. The launcher uses `@noindex` to stay part of the extension package.
